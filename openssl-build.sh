...
then
    ...
    ./Configure android-armv7 shared no-ssl2 no-ssl3 no-comp no-hw no-engine no-threads no-asm --openssldir=${OPENSSL_OUTPUT_PATH}
    build_library
...
then
    ...
    export LDFLAGS=" ${ARCH_LINK} "
    cd ${OPENSSL_TMP_FOLDER}
    ./Configure android shared no-ssl2 no-ssl3 no-comp no-hw no-engine no-threads no-asm --openssldir=${OPENSSL_OUTPUT_PATH}
    build_library
...
then
    ...
    export LDFLAGS=" ${ARCH_LINK} "
    cd ${OPENSSL_TMP_FOLDER}
    ./Configure android-x86 shared no-ssl2 no-ssl3 no-comp no-hw no-engine no-threads no-asm --openssldir=${OPENSSL_OUTPUT_PATH}
    build_library
...
then
    ...
    export LDFLAGS=" ${ARCH_LINK} "
    cd ${OPENSSL_TMP_FOLDER}
    ./Configure linux-x86_64 shared no-ssl2 no-ssl3 no-comp no-hw no-engine no-threads no-asm --openssldir=${OPENSSL_OUTPUT_PATH}
    build_library
